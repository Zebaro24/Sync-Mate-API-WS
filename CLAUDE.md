...
Гид для Claude по бэкенд-части. **Полная документация — в [docs/](docs/)** (архитектура, REST API, WS-протокол, Rezka, конфигурация, тесты, деплой, конвенции, ADR). Держи её в синхроне с кодом (агент `docs-sync` / `/finish`).
...
Полный список сообщений — в `docs/websocket-protocol.md`. Запомните минимум:
